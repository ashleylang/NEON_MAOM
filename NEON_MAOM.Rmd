---
title: 'Patterns in mineral-associated organic matter across eastern US NEON forest sites'
output:
...
min_N_HF=min(HF_CN$pct.N.soil)
  max_N_HF=max(HF_CN$pct.N.soil)
...
ggsave("Figure_1.pdf", path="figures/", width= 18, height= 9, dpi=600, units="cm")
...
#Multiple regression models--does not change results! Site not an important factor.
...
#LENO is an outlier to the proportion of C in MAOM:
c.prop.mod.hf.leno=lm(proportion.C_HF ~  E,
           data=final_data %>% filter(Site=="LENO"),na.action = na.omit)
summary(c.prop.mod.hf.leno)
...
tab_model(c.mod5, c.mod6,n.mod5, n.mod6, show.se = TRUE, show.std=TRUE, show.ci = FALSE, digits = 2, digits.re = 2,show.df=TRUE)
...
ggsave("Figure_2.pdf",path="figures/", dpi=600, width= 30, height= 10, units="cm")

ggarrange(propN.myc.flf, propN.myc.olf, propN.myc, nrow=1, ncol=3, common.legend=T, legend="right", labels=c("a", "b","c"))
ggsave("Figure_S1.pdf",path="figures/", dpi=600, width= 30, height= 10, units="cm")
...
myc_gradient_prophf_coefficients=summary(lmList(proportion.C_HF~E|Site, data=final_data))$coefficients %>% 
  as.data.frame()
# myc_gradient_prophf_slopes=coef(lmList(proportion.C_HF~E|Site , data = final_data))[2]
Site=c("TREE", "BART", "HARV", "SERC", "ORNL", "DELA", "LENO")
c.mod1.slopes= data.frame(Site)%>% 
  cbind(myc_gradient_prophf_coefficients) %>% 
  rename("Slope"="Estimate.E",
         "SE"= "Std. Error.E") %>% 
  left_join(CDI, by="Site")

#Now for MAOM C concentrations
myc_gradient_conchf_coefficients=summary(lmList(mg.C.per.g.soil_HF~E|Site, data=final_data))$coefficients %>% 
  as.data.frame()

c.mod2.slopes= data.frame(Site) %>%  
  cbind(myc_gradient_conchf_coefficients) %>% 
  rename("Slope"="Estimate.E",
         "SE"= "Std. Error.E") %>% 
  left_join(CDI, by="Site")

#For MAOM N proportion:
myc_gradient_propNhf_coefficients=summary(lmList(proportion.N_HF~E|Site, data=final_data))$coefficients %>% 
  as.data.frame()

n.mod1.slopes= data.frame(Site) %>%  
  cbind(myc_gradient_propNhf_coefficients) %>% 
  rename("Slope"="Estimate.E",
         "SE"= "Std. Error.E") %>% 
  left_join(CDI, by="Site")

myc_gradient_concNhf_coefficients=summary(lmList(mg.N.per.g.soil_HF~E|Site , data = final_data))$coefficients %>% 
  as.data.frame()

n.mod2.slopes= data.frame(Site) %>%  
  cbind(myc_gradient_concNhf_coefficients) %>% 
  rename("Slope"="Estimate.E",
         "SE"= "Std. Error.E") %>% 
  left_join(CDI, by="Site")

w=ggplot(aes(x=CDI, y=Slope, label=Site),data=c.mod1.slopes)+
  geom_point(size=2.5)+
  geom_errorbar(data=c.mod1.slopes, aes(ymin=Slope-SE, ymax=Slope+SE))+
  geom_text_repel()+
...
x=ggplot(aes(x=CDI, y=Slope, label=Site),data=c.mod2.slopes)+
  geom_point(size=2.5)+
    geom_errorbar(data=c.mod2.slopes, aes(ymin=Slope-SE, ymax=Slope+SE))+
  geom_text_repel()+
...
y=ggplot(aes(x=CDI, y=Slope, label=Site),data=n.mod1.slopes)+
  geom_point(size=2.5)+
    geom_errorbar(data=n.mod1.slopes, aes(ymin=Slope-SE, ymax=Slope+SE))+
  geom_text_repel()+
...
z=ggplot(aes(x=CDI, y=Slope, label=Site),data=n.mod2.slopes)+
  geom_point(size=2.5)+
    geom_errorbar(data=n.mod2.slopes, aes(ymin=Slope-SE, ymax=Slope+SE))+
  geom_text_repel()+
  geom_hline(yintercept=0, linetype="dashed")+
...
ggarrange(w,y,x,z, labels=c("a", "b", "c", "d"), nrow=2, ncol=2, legend=F)
ggsave("Figure_3.pdf", path="figures/", dpi=600, width= 20, height= 18, units="cm")
...
ggsave("Figure_4.pdf",path="figures/",dpi=600,  width= 20, height= 10, units="cm")
...
#from -26.346 at the lowest CDI to -26.349 at the highest CDI.
...
ggsave("Figure_6.pdf",path="figures/", dpi=600, width= 14, height= 10, units="cm")
...
ggsave("Figure_S3.pdf", path="figures/", width= 24, height= 14, units="cm")
...
#ggsave("pctAngioGymno.pdf", path="figures/", width= 20, height= 16, units="cm")
...
#ggsave("FigS.pdf", path="figures/", width= 24, height= 12, units="cm")
...
#using neon_full_data to plot % AM and pH at LENO
LENO_plot=ggplot(data=neon_full_data %>% filter(Site=="LENO"), aes(x=A, y=pH))+
...
ggsave("Figure_5.pdf",path="figures/", dpi=600,  width= 30, height= 10, units="cm")
